...
#ifndef _MUNK_HTMLPRINT_H_
#define _MUNK_HTMLPRINT_H_
...
class MunkHtmlDCRenderer : public wxObject
{
public:
    MunkHtmlDCRenderer();
    virtual ~MunkHtmlDCRenderer();
...
private:
    wxDC *m_DC;
    MunkHtmlParsingStructure *m_Parser;
    wxFileSystem *m_FS;
    MunkHtmlContainerCell *m_Cells;
    int m_MaxWidth, m_Width, m_Height;

    DECLARE_NO_COPY_CLASS(MunkHtmlDCRenderer)
};
...
class WXDLLIMPEXP_HTML MunkHtmlPrintout : public wxPrintout
{
public:
    MunkHtmlPrintout(const wxString& title = wxT("Printout"));
    virtual ~MunkHtmlPrintout();
    ...
    void SetHeader(const wxString& header, int pg = MunkHtmlPAGE_ALL);
    void SetFooter(const wxString& footer, int pg = MunkHtmlPAGE_ALL);
            // sets header/footer for the document. The argument is interpreted as HTML document.
            // You can use macros in it:
            //   @PAGENUM@ is replaced by page number
            //   @PAGESCNT@ is replaced by total number of pages
            //
            // pg is one of MunkHtmlPAGE_ODD, MunkHtmlPAGE_EVEN and wx_PAGE_ALL constants.
    ...
    // Adds input filter
    // static void AddFilter(MunkHtmlFilter *filter);
...
private:
    int m_NumPages;
    //int m_PageBreaks[MunkHTML_PRINT_MAX_PAGES];
    ...
    MunkHtmlDCRenderer *m_Renderer, *m_RendererHdr;
    float m_MarginTop, m_MarginBottom, m_MarginLeft, m_MarginRight, m_MarginSpace;

    // list of HTML filters
    static wxList m_Filters;

    DECLARE_NO_COPY_CLASS(MunkHtmlPrintout)
};
...
class WXDLLIMPEXP_HTML MunkHtmlEasyPrinting : public wxObject
{
public:
    MunkHtmlEasyPrinting(const wxString& name = wxT("Printing"), wxWindow *parentWindow = NULL);
    virtual ~MunkHtmlEasyPrinting();
...
protected:
    virtual MunkHtmlPrintout *CreatePrintout();
    virtual bool DoPreview(MunkHtmlPrintout *printout1, MunkHtmlPrintout *printout2);
    virtual bool DoPrint(MunkHtmlPrintout *printout);
...
private:
    ...
    DECLARE_NO_COPY_CLASS(MunkHtmlEasyPrinting)
};




#endif // _MUNK_HTMLPRINT_H_
